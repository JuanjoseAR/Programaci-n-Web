...
class PassengerServicesImplTest {
    ...
    void shouldCreatePassengerWithProfile() {
        ...
        when(passengerMapper.toResponse(any())).thenReturn(response);
    ...
    void shouldUpdateExistingPassenger() {
        var profile = PassengerProfile.builder().phone("111").countryCode("+11").build();
        var passenger = Passenger.builder().id(5L).fullName("Old").email("old@test.com").profile(profile).build();

        var req = new PassengerUpdateRequest( "New","old@test.com", new PassengerProfileDto("222", "+22"));
        var updatedResponse = new PassengerResponse(5L, "New", "old@test.com",
                new PassengerProfileDto("222", "+22"));

        when(passengerRepository.findByEmailIgnoreCase("old@test.com")).thenReturn(Optional.of(passenger));
        when(profileRepository.save(any())).thenReturn(profile);
        when(passengerRepository.save(any())).thenReturn(passenger);
        when(passengerMapper.toResponse(any())).thenReturn(updatedResponse);
    ...
    void shouldGetPassengerByEmail() {
        var passenger = Passenger.builder().id(7L).fullName("Ana").email("ana@test.com").build();
        var response = new PassengerResponse(7L, "Ana", "ana@test.com", null);
    ...
    void shouldGetPassengerById() {
        var passenger = Passenger.builder().id(3L).fullName("Carlos").email("c@test.com").build();
        var response = new PassengerResponse(3L, "Carlos", "c@test.com", null);

        when(passengerRepository.findById(3L)).thenReturn(Optional.of(passenger));
        when(passengerMapper.toResponse(passenger)).thenReturn(response);
    ...
    void shouldListPassengersPaged() {
        var passenger = Passenger.builder().id(1L).fullName("A").email("a@test.com").build();
        var response = new PassengerResponse(1L, "A", "a@test.com", null);
        var page = new PageImpl<>(java.util.List.of(passenger));

        when(passengerRepository.findAll(PageRequest.of(0, 5))).thenReturn(page);
        when(passengerMapper.toResponse(passenger)).thenReturn(response);
